...
async fn run_retranscription<R: Runtime>(
    ...
    // VAD can identify audio as speech while the selected model returns only
    // empty strings. Treat that as a failed retranscription before opening the
    // replacement transaction so the existing transcript is never deleted.
    ensure_usable_transcripts(transcribed_count)?;
...
fn ensure_usable_transcripts(transcribed_count: usize) -> Result<()> {
    if transcribed_count == 0 {
        Err(anyhow!(
            "No usable speech was transcribed; the existing transcript was preserved"
        ))
    } else {
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn empty_model_output_is_rejected_before_replacement() {
        let error = ensure_usable_transcripts(0).unwrap_err().to_string();
        assert!(error.contains("existing transcript was preserved"));
        assert!(ensure_usable_transcripts(1).is_ok());
    }
